...
function useAppState() {
  ...
  useEffect(() => { for (const raw of messages.slice(processed.current)) { const msg = raw as any;
    ...
    else if (["thinking", "tool", "result", "error"].includes(msg.type)) { const sid = msg.payload.sessionId; setWorkspaces((items) => items.map((w) => ({ ...w, sessions: w.sessions.map((s) => s.id === sid ? { ...s, messages: [...s.messages, { id: crypto.randomUUID(), type: msg.type, payload: msg.payload }] } : s) }))); }
  ...
  const sendMessage = (message: string) => { if (!activeSessionId) return; setWorkspaces((items) => items.map((w) => ({ ...w, sessions: w.sessions.map((s) => s.id === activeSessionId ? { ...s, messages: [...s.messages, { id: crypto.randomUUID(), type: "user", payload: { message } }] } : s) }))); send({ type: "add-message", payload: { sessionId: activeSessionId, message } }); };
  return useMemo(() => ({ socket, loading, workspaces, activeWorkspace, activeSession, activeWorkspaceId, activeSessionId, setActiveWorkspaceId, setActiveSessionId, addWorkspace, addSession, sendMessage }), [socket, loading, workspaces, activeWorkspace, activeSession, activeWorkspaceId, activeSessionId]);
}
